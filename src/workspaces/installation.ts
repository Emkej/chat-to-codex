...
function installationFile(stateDir: string): string {
  return path.join(stateDir, "installation.json");
}

/** Load an existing installation identity without creating any state. */
export function loadInstallationIfExists(stateDir: string): InstallationIdentity | null {
  const existing = readJsonIfExists<InstallationIdentity>(installationFile(stateDir));
...
export function loadOrCreateInstallation(stateDir: string): InstallationIdentity {
  const existing = loadInstallationIfExists(stateDir);
