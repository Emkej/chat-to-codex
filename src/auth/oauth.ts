...
/**
 * Auto-submits the pairing form as soon as a complete code is entered
 * (pasted or typed), and shows submit state on the button. Served inline
 * under a matching CSP hash — see setAuthSecurityHeaders.
 */
export const PAIRING_AUTO_SUBMIT_SCRIPT = `
(function () {
  var form = document.querySelector("form");
  var input = document.getElementById("pairing_code");
  var button = form ? form.querySelector("button") : null;
  if (!form || !input) return;
  var count = function () { return input.value.replace(/[^A-Za-z0-9]/g, "").length; };
  var submitted = false;
  var submit = function () {
    if (submitted || count() < 8) return;
    submitted = true;
    if (button) { button.disabled = true; button.textContent = "Connecting…"; }
    form.submit();
  };
  input.addEventListener("input", function () { window.setTimeout(submit, 0); });
  input.addEventListener("paste", function () { window.setTimeout(submit, 0); });
})();`;
...
<script>${PAIRING_AUTO_SUBMIT_SCRIPT}</script>
</body>
</html>`;
}
